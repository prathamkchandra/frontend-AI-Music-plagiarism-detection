...
interface ResultCardProps {
  final_score: number;
  uploaded_song: string;
  uploaded_song_path: string;
  match_song: string;
  match_song_path: string;
  match_label: string;
}
  ...
  return (
    <div className="bg-gray-900/80 backdrop-blur-md mt-6 p-6 rounded-2xl shadow-xl max-w-3xl w-full mx-auto text-wrap break-all">
      ...
      <p className="mb-2 p-3">
        {" "}
        Uploaded Song: <b>{data.uploaded_song}</b>
      </p>
      {data.uploaded_song_path && (
        <audio
          controls
          src={data.uploaded_song_path}
      ...
      {data.match_song && (
        <p className="mb-2 text-wrap overflow-hidden word-wrap break-all">
          Matched Song: <b>{data.match_song}</b>{" "}
          <span className="text-gray-400">({data.match_label})</span>
        </p>
      )}
      {data.match_song_path && (
        <audio
          controls
          src={data.match_song_path}
          className="w-full mb-4 rounded-lg"
        />
      )}
    </div>
